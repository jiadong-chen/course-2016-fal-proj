...
class proj2(dml.Algorithm):
    ...
    def execute(trial=True):
        ...
        for (i,j) in P:
            garden_location.append({'location':(i,j)})
        ...
        if trial==False:
            crime=[(j,i) for (i,j) in ra.sample(crime_test,1500) if (i,j)!=(0,0)]
        else:
            crime=[(j,i) for (i,j) in ra.sample(crime_test,1000) if (i,j)!=(0,0)]
        ...
        Temp_crime=K_means(X_random,crime)
        
        #print ("K-means of crime locations done")
        Crime_cluster=[]
        #for (i,j) in Temp:
        for (i,j) in Temp_crime:
            Crime_cluster.append({'location':(i,j)})
        with open("crime.json",'w') as outfile:
            json.dump(Crime_cluster,outfile,indent=4,sort_keys=True)
        print ("k-means of crime done")
        ...
        #print (crime)
        '''
        from_garden_to_crime=closest(P,Temp_crime)
        std_garden_to_crime=np.std(from_garden_to_crime)
        from_crime_to_garden=closest(Temp_crime, P)
        std_crime_to_garden=np.std(from_crime_to_garden)

        print ("This is the distance distribution between garden and crime")
        print (from_garden_to_crime)
        print ("This is the distance distribution between crime and garden")
        ...
        from_garden_cluster_to_crime=closest(K,Temp_crime)
        std_garden_cluster_to_crime=np.std(from_garden_cluster_to_crime)
        from_crime_to_garden_cluster=closest(Temp_crime, K)
        std_crime_to_garden_cluster=np.std(from_crime_to_garden_cluster)
        print ("This is the distance distribution between garden cluster and crime")
        new_from_garden_cluster_to_crime=[]
        for i in from_garden_cluster_to_crime:
            i=(i-min(from_garden_cluster_to_crime))/(max(from_garden_cluster_to_crime)-min(from_garden_cluster_to_crime))
            new_from_garden_cluster_to_crime.append(i)
        print (new_from_garden_cluster_to_crime)
        print ("This is the distance distribution between crime and garden cluster")
        new_from_crime_to_garden_cluster=[]
        for i in from_crime_to_garden_cluster:
            i=(i-min(from_crime_to_garden_cluster))/(max(from_crime_to_garden_cluster)-min(from_crime_to_garden_cluster))
            new_from_crime_to_garden_cluster.append(i)
        print (new_from_crime_to_garden_cluster)
        '''
        print ("This is the standard deviation between garden cluster and crime:")
        print (std_crime_to_garden_cluster)
        print (std_garden_cluster_to_crime)
        '''
        #print (liquor_tuple)
        #print (crime)
        from_liquor_to_crime=closest(liquor_tuple,Temp_crime)
        std_liquor_to_crime=np.std(from_liquor_to_crime)
        from_crime_to_liquor=closest(Temp_crime, liquor_tuple)
        std_crime_to_liquor=np.std(from_crime_to_liquor)
        print ("This is the distance distribution between liquor and crime")
        new_from_liquor_to_crime=[]
        for i in from_liquor_to_crime:
            i=(i-min(from_liquor_to_crime))/(max(from_liquor_to_crime)-min(from_liquor_to_crime))
            new_from_liquor_to_crime.append(i)
        print (new_from_liquor_to_crime)
        print ("This is the distance distribution between crime and liquor")
        new_from_crime_to_liquor=[]
        for i in from_crime_to_liquor:
            i=(i-min(from_crime_to_liquor))/(max(from_crime_to_liquor)-min(from_crime_to_liquor))
            new_from_crime_to_liquor.append(i)
        print (new_from_crime_to_liquor)
        '''
        ...
        #print ("This are the richest property locations in Boston:")
        #print (richlist_test)
        richlist_cluster=[]
        for (i,j) in richlist_test:
            richlist_cluster.append({'location':(i,j)})
        #print (richlist_test)
        #print (crime)
        from_richlist_to_crime=closest(richlist_test,Temp_crime)
        std_richlist_to_crime=np.std(from_richlist_to_crime)
        from_crime_to_richlist=closest(Temp_crime, richlist_test)
        std_crime_to_richlist=np.std(from_crime_to_richlist)
        print ("This is the distance distribution between high_valued prop and crime")
        new_from_richlist_to_crime=[]
        for i in from_richlist_to_crime:
            i=(i-min(from_richlist_to_crime))/(max(from_richlist_to_crime)-min(from_richlist_to_crime))
            new_from_richlist_to_crime.append(i)
        print (new_from_richlist_to_crime)
        print ("This is the distance distribution between crime and high_valued prop")
        new_from_crime_to_richlist=[]
        for i in from_crime_to_richlist:
            i=(i-min(from_crime_to_richlist))/(max(from_crime_to_richlist)-min(from_crime_to_richlist))
            new_from_crime_to_richlist.append(i)
        print (new_from_crime_to_richlist)
